...
function getCategorySynonyms(category: string): string[] {
    switch (category) {
        ...
        case "updates":
            return ["updates", "update", "notification", "alert"];
        case "legal":
            return ["legal", "law", "attorney", "lawyer", "contract", "agreement"];
        case "healthcare":
            return ["healthcare", "health", "medical", "doctor", "hospital", "clinic"];
        case "promotions":
            return ["promotions", "promotion", "discount", "offer", "deal", "sale"];
        case "marketing":
            return ["marketing", "advertisement", "ad", "campaign", "promotional"];
        case "scheduling":
            return ["scheduling", "schedule", "appointment", "booking", "calendar"];
        case "shipping":
            return ["shipping", "ship", "delivery", "tracking", "package", "order"];
        case "support":
            return ["support", "help", "assistance", "customer service", "ticket"];
        case "alerts":
            return ["alerts", "alert", "notification", "warning", "important"];
        case "educational":
            return ["educational", "education", "learning", "course", "training", "tutorial"];
        case "technology":
            return ["technology", "tech", "software", "app", "digital", "computer"];
        case "food":
            return ["food", "restaurant", "dining", "meal", "delivery", "takeout"];
        case "entertainment":
            return ["entertainment", "entertain", "movie", "music", "game", "fun"];
        case "security":
            return ["security", "secure", "password", "login", "authentication", "verification"];
        default:
            return [category];
    }
}
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        // Find ALL emails missing AI metadata or with invalid AI metadata, not just one per thread
        const emailsNeedingEnhancement = decryptedEmails.filter((email) => {
            // Check if email has no AI metadata
            if (!email.aiMetadata) {
                return true;
            }
            
            // Check if AI metadata is invalid (NULL category, empty summary, etc.)
            const metadata = email.aiMetadata;
            const hasInvalidMetadata = 
                !metadata.category || 
                metadata.category === "Uncategorized" ||
                !metadata.summary || 
                metadata.summary === "No summary available" ||
                metadata.summary === "Error processing email" ||
